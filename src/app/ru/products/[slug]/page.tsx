...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  try {
    const service = await getServiceBySlug(slug, 'ru')
    const url = `https://antalyaagacbudama.com/ru/products/${slug}`
    return {
      title: service.title,
      ...
      alternates: {
        ...
        languages: {
          ...
          'en': `https://antalyaagacbudama.com/en/products/${slug}`,
          'ru': url,
...
export default async function RuServicePage({ params }: Props) {
  const { slug } = await params

  let service
  try {
    service = await getServiceBySlug(slug, 'ru')
  } catch {
    notFound()
  }
      ...
      <section className="bg-[#f5f2ed] py-16 md:py-24">
        <div className="container mx-auto px-6 max-w-4xl">
          <nav className="flex items-center gap-2 text-sm text-gray-400 mb-12">
            <Link href="/ru" className="hover:text-green-600 transition-colors flex items-center gap-1.5">
            ...
            <Link href="/ru/products" className="hover:text-green-600 transition-colors">
              Наши услуги
            </Link>
            <svg className="w-3.5 h-3.5 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2">
              <path strokeLinecap="round" strokeLinejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5" />
          ...
          <article className="bg-white rounded-2xl p-8 md:p-12 shadow-sm">
            ...
            <div className="mt-10 pt-8 border-t border-gray-100 flex flex-col sm:flex-row items-start sm:items-center gap-4">
              ...
              >
                Получить оценку для этой услуги
              </Link>
              <a
